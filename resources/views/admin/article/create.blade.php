<x-dashboard-layout>
    ...
    <form method="POST" action="{{ route('dashboard.articles.store') }}">
        ...
        <div class="mb-5">
            ...
            <input type="title" id="title" class="bg-slate-secondary-medium border border-slate-200 text-heading text-sm rounded-lg focus:ring-brand focus:border-brand block w-full px-3 py-2.5 shadow-xs" name="title" value="{{ old('title') }}" placeholder="Give audience briefly..." />
        ...
        <div class="mb-5">
            ...
            <textarea id="content" rows="4" class="bg-slate-secondary-medium border border-slate-200 text-heading text-sm rounded-lg focus:ring-brand focus:border-brand block w-full p-3.5 shadow-xs" placeholder="Write your thoughts here..." name="content">{{ old('content') }}</textarea>
            @error('content')
                <p class="mt-1 text-sm text-red-500"><span class="font-medium">
                    {{ $message }}
                </p>
